...
bool get_longitudinal_allowed(bool allow_long_with_gas_override) {
  return controls_allowed && controls_allowed_long && (!gas_pressed_prev || allow_long_with_gas_override);
}
...
bool longitudinal_accel_checks(int desired_accel, const LongitudinalLimits limits) {
  bool accel_valid = get_longitudinal_allowed(limits.allow_long_with_gas_override) && !max_limit_check(desired_accel, limits.max_accel, limits.min_accel);
  bool accel_inactive = desired_accel == limits.inactive_accel;
  return !(accel_valid || accel_inactive);
}

bool longitudinal_speed_checks(int desired_speed, const LongitudinalLimits limits) {
  return !get_longitudinal_allowed(limits.allow_long_with_gas_override) && (desired_speed != limits.inactive_speed);
}

bool longitudinal_transmission_rpm_checks(int desired_transmission_rpm, const LongitudinalLimits limits) {
  bool transmission_rpm_valid = get_longitudinal_allowed(limits.allow_long_with_gas_override) && !max_limit_check(desired_transmission_rpm, limits.max_transmission_rpm, limits.min_transmission_rpm);
...
bool longitudinal_gas_checks(int desired_gas, const LongitudinalLimits limits) {
  bool gas_valid = get_longitudinal_allowed(limits.allow_long_with_gas_override) && !max_limit_check(desired_gas, limits.max_gas, limits.min_gas);
...
bool longitudinal_brake_checks(int desired_brake, const LongitudinalLimits limits) {
  ...
  violation |= !get_longitudinal_allowed(limits.allow_long_with_gas_override) && (desired_brake != 0);
...
bool longitudinal_interceptor_checks(const CANPacket_t *to_send, const LongitudinalLimits limits) {
  return !get_longitudinal_allowed(limits.allow_long_with_gas_override) && (GET_BYTE(to_send, 0) || GET_BYTE(to_send, 1));
}
